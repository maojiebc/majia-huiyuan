...
> 一套**完整的、可以照着抄的**连锁会员数据体系。以一家虚构的咖啡连锁为例，从会员注册的第一行数据，到老板看的经营驾驶舱：**54 个数据集、25 条数据加工链、12 张看板**，全部摊开。
>
...
**② 数据同学：抄字段和口径。** [结构定义](./数据集/结构定义/)里有全部字段，[逻辑SQL](./ETL/逻辑SQL/) 里有全部加工口径："顾客识别键怎么建""RFM 怎么打分""回本偏离度怎么算"，直接抄，表名换成你家的就能用。SQL 方言是 Spark 3.4。
...
| 项目 | 关系 |
|---|---|
| [majia-guanyuan](https://github.com/maojiebc/majia-guanyuan) | 观远 BI 实战增益层 skill。**工具在那边，数据在这边**；本库最初是它的实践案例，现独立迭代 |
| [restaurant-bi-formulas](https://github.com/maojiebc/majia-guanyuan/tree/main/references/restaurant-bi-formulas) | 餐饮 BI 公式实战库：复购 / RFM / 核销的 SQL 公式大全（2900 行），配合本库食用 |
