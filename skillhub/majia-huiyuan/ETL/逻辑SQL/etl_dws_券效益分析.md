...
3. 若同一订单异常关联多张券实例，GMV 只归给优惠金额最大的券；并列时按 `券ID` 排序。归因 CTE 名称必须是 `bridge_券核销订单`，口径对齐 `ETL/公共口径/02_券实例核销订单桥.sql`。生产落地时应先物化公共桥再引用。
...
- `bridge_券核销订单` 中 `订单ID` 必须唯一，且每行必须能回溯到唯一 `券ID`。
