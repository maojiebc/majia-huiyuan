> **v1.4.1 结构覆盖：**当前输出 56 列：在下方 v1.4.0 的 51 列上新增
> `投资起始日 DATE`、`剩余回本月数 INT`、`回本状态 STRING`、`预计完整回本日期 DATE`、
> `数据快照日期 DATE`。利润月份只命中同期有效加盟合同；加盟商和回本关联均显式投影，
> 不再由 `SELECT *` 带入重复主键或无关字段。下方平台字段是 v1.4.0 workshop 快照。
