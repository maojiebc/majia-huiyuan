...
| 搭观远 BI，想整套导入 | [ETL/原始JSON/](./ETL/原始JSON/) + [数据集/原始JSON/](./数据集/原始JSON/)（注意：资源 ID 为实例私有，复刻时须重新生成） |
...
> **复刻这套体系，先填这三张表。**  
> `param_` 三张参数表是整套 ETL 的阈值配置中心：哪些日期算豁免不报警、多少天不来算沉睡、利润率多少算健康，都写在这里——而不是硬编码进公式。填完它们，再跑 ETL，才不会在自己的数据上触发错误的告警或分层。结构定义在 [数据集/结构定义/](./数据集/结构定义/)，搜 `param_` 前缀即可找到三张表。
...
**② 数据同学：抄字段和口径。** [结构定义](./数据集/结构定义/)里有全部字段，[逻辑SQL](./ETL/逻辑SQL/) 里有全部加工口径，[公式库](./公式库/) 里有复购 / RFM（按最近来没来、来得勤不勤、花得多不多给顾客分层）/ 核销这些通用算法的标准 SQL："顾客识别键怎么建""RFM 怎么打分""回本偏离度怎么算"，直接抄，表名换成你家的就能用。SQL 方言是 Spark 3.4。
